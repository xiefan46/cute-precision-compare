...
def test_kv_match_cute_F32F16F16F32_torch_f16_amp(k, v, myflash):
    assert k.dtype == torch.float16
    assert v.dtype == torch.float16
    BLOCK = 64
    B, H, N, d = k.shape
    ...
    def torch_compute_kv_all_f32():
        ...
        with autocast():
            for i in range(num_block):
                ...
                ki = k[:, :, si:ei].contiguous()
                vi = v[:, :, si:ei].contiguous()
                assert ki.dtype == torch.float16
                assert vi.dtype == torch.float16
                new_kv = torch.matmul(ki.transpose(-1, -2), vi).to(torch.float32)
                kv = kv + new_kv
                kv_output[i] = kv.detach().clone()
                # print(f"data types. ki : {ki.dtype}, vi : {vi.dtype},  new_kv: {new_kv.dtype}, kv: {kv.dtype}")
...
if __name__ == "__main__":
    ...
    # cace5 cute F16F16F16F16, torch all F16, disable amp explicitly, not match
